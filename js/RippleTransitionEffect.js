class RippleTransitionEffect extends EffectShell {
    ...
    init() {
        ...
        `,
            ...
            vec3 transition(vec2 uv) {
                // basically:  https://github.com/gl-transitions/gl-transitions/blob/master/transitions/ripple.glsl

                vec2 dir = uv - vec2(.5);
                float dist = length(dir);
                vec2 offset = dir * (sin(uMixValue * dist * amplitude - uMixValue * speed) + .5) / 30.;

                return mix(
                    texture2D(uPreviousTexture, uv + offset).rgb,
                    texture2D(uTexture, vUv).rgb,
                    smoothstep(0.2, 1.0, uMixValue)
                );
            }
    ...
    onTargetChange(index) {
        ...
        TweenLite.to(this.uniforms.uMixValue, 1.0, {
            value: 1.0,
            ease: Power0.easeOut
        });
    }
